...
class AgendaPagerAdapter(private val mAgenda: List<DaySchedule>, private val activity: Activity) : PagerAdapter() {
    ...
    override fun instantiateItem(collection: ViewGroup, position: Int): Any {
        ...
        val adapter = ScheduleDayAdapter(activity, getItems(position), true, ScheduleDayAdapter.OnItemClickListener { agendaItem -> SessionDetailActivity.startActivity(activity, agendaItem) })
        ...
        if (position >= 0) {
            if (animate) {
                ...
                layoutManager!!.scrollToPositionWithOffset(position,
                        this.activity.resources.getDimensionPixelSize(R.dimen.default_padding))
            }
        }
    }
